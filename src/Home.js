import React from 'react';
import './Home.css';
import Product from './Product'

function Home() {
    return (
        <div className="home">
            <div className="home__container">
                <img className="home__image"
                    src="https://store-images.s-microsoft.com/image/apps.16285.14618985536919905.552c0017-6644-49a8-8467-8f7b34ce0428.30ad6b05-16d9-4d5e-a242-43107708a16a?mode=scale&q=90&h=1080&w=1920" alt=""/>
                <div className="home__row">
                    <Product image="https://images-na.ssl-images-amazon.com/images/I/51b7XbfMIIL.jpg"
                        title="Clean Code"
                        price={33.90}
                        rating={5}/>
                    <Product title="Sony WH-1000XM4 Wireless Industry Leading Noise Canceling Overhead Headphones with Mic for Phone-Call and Alexa Voice Control, Black"
                        image="data:image/jpeg;base64,"
                        price={349.99}
                        rating={5}/>
                </div>
                <div className="home__row">
                    <Product title="Dell UltraSharp 43 4K USB-C Monitor - U4320Q"
                    image="https://i.dell.com/is/image/DellContent//content/dam/global-asset-library/Products/peripherals_dell/Monitors/ultrasharp/u4320q/u4320q_csy_0000ff090_gy.psd?fmt=pjpg&pscan=auto&scl=1&hei=402&wid=682&qlt=85,0&resMode=sharp2&op_usm=1.75,0.3,2,0&size=682,402"
                    price={864.99}
                    rating={4}/>
                    <Product title="Apple M1 Chip with 8‑Core CPU and 7‑Core GPU 256GB Storage"
                    image="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/macbook-air-space-gray-select-201810?wid=904&hei=840&fmt=jpeg&qlt=80&op_usm=0.5,0.5&.v=1603332211000"
                    price="999.09"
                    rating={4}/>
                    <Product title="Latest Blue Yeti USB Microphone with 4 Pickup Patterns, 3 Condenser Capsules for Gaming, Streaming, Podcasting on PC/Mac, with GalliumPi Pop Filter and Microfiber Clothing Bundle (Blackout)"
                    image="https://m.media-amazon.com/images/I/61v94IqmUwL._AC_UY218_.jpg"
                    price={159.99}
                    rating={5}/>
                </div>
                <div className="home__row">
                    <Product title="Retro Round Blue Light Blocking Glasses Women/Men - FEIDU Computer Anti Eye Eyestrain Reading Gaming Fashion Glasses Non Prescription2020 New"
                    image="https://m.media-amazon.com/images/I/51DYiWkmgXL._AC_UL320_.jpg"
                    price={10.92}
                    rating={4}/>
                    <Product title="Sparkly Rhinestone Mesh Face Mask Masquerade Mask for Women Glitter Face Mask Bling Face Mask Christmas Party"
                    image="https://m.media-amazon.com/images/I/81LeJdaoQBL._AC_UL320_.jpg"
                    price={10.92}
                    rating={2} />
                    <Product title="TeeTurtle | The Original Reversible Octopus Plushie | Patented Design | Black and Gray | Show your mood without saying a word!"
                    image="https://images-na.ssl-images-amazon.com/images/I/41NRm6RmoPL._AC_UL320_SR320,320_.jpg"
                    price={15.00}
                    rating={5}/>
                </div>
            </div>
        </div>
    )
}

export default Home
